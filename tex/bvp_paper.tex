...
In a high Pe system, many convective dynamical timescales must pass before 
...
Fully implicit methods cannot be used to address the stiffness resulting from
the discrepancy between convective and thermal timescales; convective flows are
highly nonlinear, and implicit methods are bound by the nonlinear flow timescales
...
solutions to solutions that are evolved through a thermal diffusion timescale.
Finally,
...
The converged fluxes achieved through AE is at most 5\% different from the SE solution, 
...
$\xi$ is a general depth-dependent scaling factor which
tells us roughly how much larger F$_\text{E}$ is in the interior than it should
be in the thermally equilibrated state (e.g., in Fig.~\ref{fig:time_trace}b,
F$_{\text{tot}} \sim$ F$_{\text{E}} \sim 15\mathcal{P}$ just below the upper boundary layer, 
but in the equilibrated state 
it should just be $\mathcal{P}$, so $\xi \sim 1/15$ at that depth).
...
As the AE BVP solve only adjusts $\angles{T}$ and $\angles{\varpi}$, we must
also appropriately adjust the temperature perturbations around the mean and the
convective velocity flows, or else F$_{\text{E}}$ would be left unchanged, and
would still be in a strongly disequilibrium state (e.g., Fig.~\ref{fig:time_trace}b).
...
flux they carry, appropriately. This scaling of variables with $\sqrt{\xi}$ is also
the reason that we multiply $\angles{\bm{u}\times\bm{\omega}}_{x, y}$ by $\xi$
to construct $\angles{\bm{u}\times\bm{\omega}}_{x, y\text{, ev}}$.
...
Nusselt number in SE (a) and AE (b) for the simulations shown in 
Fig.~\ref{fig:time_trace}. The mean value of Nu (black) is shown vs. time in both
panels, and a centered boxcar moving average with a span of 50 freefall time
units is overplotted as a thick, dashed line.  The mean value of the temperature
(blue) is overplotted. We see that the mean value of Nu rapidly approaches its
mean equilibrium value (plotted as a dashed horizontal black line), signifying
that the convective motions roughly reach a statistically stationary state quickly
compared to the temperature profile, which takes much longer to trend towards its
equilibrium value. Regardless, we see that the fluctuations of Nu about the mean
value are much smaller in the final equilibrated state than in the early state
which is not in thermal equilibrium, and that the post-AE flows capture this
proper Nu amplitude variation.
...
rapidly compared to the mean temperature, as shown by the thick, dashed moving
average.  As a result, if one is primarily measuring the mean value of Nu in
a convective system, a full SE run through $t_\kappa$ or the AE procedure may not
be necessary (e.g., \cite{stevens&all2010}). 
Regardless, the variations around the mean value in Nu do change significantly
over the course of thermal equilibration (Fig.~\ref{fig:nu_v_time}a). At early
times, variations in Nu about the mean are much larger than they are at later times,
and the use of AE seems to progress the fluctuations in Nu about the mean to
a state which is consistent with the equilibrated SE fluctuations.
...
\begin{table}
\caption{We provide details regarding the computational cost of select
AE and SE runs in 2D and 3D. For each run, we quote the supercriticality
($S$), the coefficient resolution (nz, nx, ny), the number of CPUs used
to perform the calculation ($N_{\text{CPU}}$), the number of CPU-hours
used to perform the run ($t_{\text{CPU,AE/SE}}$), and the ratio of CPU-hours
used in the AE run compared to the SE run.
}
\label{table:speed}
\begin{center}
\begin{tabularx}{\textwidth}{ X X X X X X }
\hline																	
$S$	&	nz, nx$\times$ny	&	$N_{\text{CPU}}$	&	
...
\hline
$10^2$	&	64,	128	&	32    & 4.369 &  2.152    & 2.03  \\
$10^3$	&	128,256	&	64    & 20.63 &  53.25    & 0.387 \\
$10^4$	&	256,512	&	128   & 180.82&  1205.25  & 0.15  \\
$10^5$	&	512,1024&	256   & 2757.73& 24061.75 & 0.115 \\
\hline \hline \multicolumn{6}{c}{\vspace{-0.2cm}}\\
\multicolumn{6}{c}{\vspace{0.1cm}3D Runs} \\
\hline
$10^1$	&	32,	64$\times$64	& 512    &   106.15      &   61.5    & 1.73\\
$10^2$	&	64,	128$\times$128	& 512    &   111.92      &   185.35  & 0.6 \\
$10^3$	&	128,256$\times$256	& 2048   &   1426.39     &   6985.18 & 0.2 \\
$10^4$	&	256,512$\times$512	& 8192   &   23063.77    &   331,717 & 0.07\\
\hline																	
\end{tabularx}
\end{center}
\end{table}
