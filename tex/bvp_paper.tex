...
In order to properly reduce F$_\text{E}$, we note that, upon removing 
terms which vanish due to symmetry, the convective enthalpy flux is carried
solely by the velocity field coupling to perturbations in temperature around
the mean temperature profile, or F$_\text{E} \propto \{w, T-\angles{T}_{x,y}\}$.
As the evolved enthalpy flux should be roughly $\xi$F$_\text{E}$, we reduce 
the velocity, $\bm{u}$,
and temperature perturbations about the mean state, $T-\angles{T}_{x,y}$,
 by $\sqrt{\xi}$. 

To properly reduce the conductive flux, we examine the 
...
\xi \angles{\bm{u}\times\bm{\omega}}_{x, y}$, where we scale 
$\angles{\bm{u}\times\bm{\omega}}_{x, y} \propto \bm{u}^2$, by $\xi$ because we
scaled $\bm{u}$ by $\sqrt{\xi}$ in order to reduce the convective enthalpy flux.
With these profiles, we solve for $\angles{\varpi}_{x,y}$ and $\angles{T_1}_{x,y}$.
Convective flows
are perturbations around a thermal profile defined by these equations in the proper evolved, 
statistically stationary state, so the proper specification of
F$_{\text{conv, ev}}$ and $\angles{\bm{u}\times\bm{\omega}}_{x,y\text{, ev}}$,
fully specifies the mean thermodynamic structure of the system. 
Our choice of fixing the temperature at the top boundary and the flux at the bottom
boundary ensures that there is a unique solution for $\angles{T_1}$ given the evolved
...
units, is overplotted as a thick, dashed line. 
...
Nu, and the overplotted thick dashed line shows a moving time average of Nu. The
time average is taken using a center boxcar window whose width is 50 $t_{\text{ff}}$.
The mean value of Nu evolves towards its final value (shown in the horizontal
dashed black line) rapidly compared to the mean system temperature (shown in
blue), which approaches its evolved state (horizontal dashed blue line) slowly.
As such, it is possible to measure the statistically stationary mean value of
Nu after a few hundred freefall times, 
as has been done previously \cite{stevens&all2010}. However, the fluctuations
around the mean value are larger at early times when the system is not thermally
equilibrated. In Fig.~\ref{fig:nu_v_time}b, we show the AE traces of Nu and mean
temperature at $S = 10^5$. The times at which AE solves are completed are marked
by arrows. Nu reaches its evolved mean value slightly more rapidly than in the SE case,
but the early, large fluctuations away from the mean that we see in the SE
case are not observed after the AE solves.
...
at $S = 10^7$ (bottom, left). We find that these systems exhibit
large Nu during
...
Computational timesaving is the primary reason to use AE rather than 
...
to 2D runs implies that 3D studies can greatly benefit from AE.



Storing information about the mean state for the AE solve marginally
decreases the rate at which our solver timesteps. However, 
...
arrive at the same solution. Studies whose horizontal boundaries are not periodic may need
a much more careful treatment of the adjustment of the mean temperature profile,
especially near the side walls.
