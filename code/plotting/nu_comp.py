...
run_df = pd.DataFrame({'t':    time1[np.argmax(nu1)+5:],
                        'nu':   nu1[np.argmax(nu1)+5:]})
print(run_df)
avg_nu1 = run_df.rolling(window=500, min_periods=50, axis=0, center=True).mean()
avg_nu1 = np.array(avg_nu1)[:,1]
ax1.plot(time1[nu1 > 0], nu1[nu1 > 0],   c='k', lw=2)
ax1.plot(run_df['t'], avg_nu1,   c='dimgrey', lw=2)#, dashes=(5,1))
#ax1.plot(time3, nu3, c='k')

run2_df = pd.DataFrame({'t':    time2[np.argmax(nu2)+2:],
                        'nu':   nu2[np.argmax(nu2)+2:]})
avg_nu2 = run2_df.rolling(window=50, min_periods=5, axis=0, center=True).mean()
avg_nu2 = np.array(avg_nu2)[:,1]

ax2.plot(time2[nu2 > 0], nu2[nu2 > 0], c='k', lw=2)
ax2.plot(run2_df['t'], avg_nu2,   c='dimgrey', lw=2)#, dashes=(5,1))

ax3.plot(time2[nu2 > 0], nu2[nu2 > 0], c='k', lw=2)
ax3.plot(run2_df['t'], avg_nu2,   c='dimgrey', lw=2)#, dashes=(5,1))
...
#TODO: make bigger, drift lower and left.
ax3.annotate("(a) SE", xy=(11280, 3e2), fontsize=16)
ax1.annotate("(b) AE", xy=(875, 3e2), fontsize=16)
