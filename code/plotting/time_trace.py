...
axes[-1].plot(z, enth,  c=f_conv_color, label=r'$F_{\mathrm{E}}$', lw=2)
axes[-1].plot(z, kappa, c=f_cond_color, label=r'$F_{\kappa}$', lw=2)
axes[-1].plot(z, sum_f, c=f_sum_color, label= r'$F_{\mathrm{tot}}$', lw=2)
y_ticks = np.array([0, 5, 10, 15, np.ceil(np.max(sum_f))])
axes[-1].set_yticks(y_ticks)
axes[-1].set_ylabel(r'$F/\mathcal{P}$')
...
axes[-1].set_yticks(y_ticks)
axes[-1].set_ylabel(r'$(F_{\mathrm{AE}} - F_{\mathrm{SE}})/\mathcal{P}$')
axes[-1].set_xlabel('z')
trans = axes[-1].get_xaxis_transform() # x in data untis, y in axes fraction
